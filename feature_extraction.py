...
probs = tf.nn.softmax(
    tf.add(tf.matmul(fc7, tf.Variable(tf.random_normal(shape=shape, mean=meanweight, stddev=standardDev))),
...
# Read Images
im1 = imread("stop.jpg").astype(np.float32)
im1 = im1 - np.mean(im1)

im2 = imread("construction.jpg").astype(np.float32)
im2 = im2 - np.mean(im2)
